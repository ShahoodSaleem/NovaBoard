...
const SyncManager = {
    ...
    async loginWithGoogle() {
        ...
        if (this._oauthLoginInProgress) {
            return { success: false, error: 'Login is already in progress. Complete the current Google sign-in window first.' };
        }

        if (typeof navigator !== 'undefined' && navigator.onLine === false) {
            return { success: false, error: 'You appear to be offline. Reconnect to the internet and try again.' };
        }

        try {
            this._oauthLoginInProgress = true;

            // Pass the current extension target URL so the hosted callback can return to the same build
            // across Chrome (chrome-extension://) and Firefox (moz-extension://).
            const extensionTargetUrl = resolveRuntimeExtensionTargetUrl();
            const extensionId = resolveRuntimeExtensionId();
            if (!extensionTargetUrl) {
                this._oauthLoginInProgress = false;
                return { success: false, error: 'Could not determine extension target. Reload LumiList and try again.' };
            }

            // Reset dedup marker before starting a fresh OAuth attempt.
            // This avoids stale callback fingerprints from blocking new login attempts.
            try {
                if (typeof chrome !== 'undefined' && chrome?.storage?.local) {
                    await chrome.storage.local.remove('_lastProcessedToken');
                }
            } catch (e) {
                console.warn('Failed to clear OAuth dedup marker before login:', e?.message || e);
            }

            const identityRuntime = this._getExtensionIdentityRuntime();
            const browserOAuthRedirectUrl = this._resolveBrowserOAuthRedirectUrl(identityRuntime);
            const oauthTransport = this._resolveGoogleOAuthTransport({
                identityRuntime,
                browserOAuthRedirectUrl,
                extensionId
            });

            if (oauthTransport.useNative) {
                return await this._startNativeGoogleLogin({
                    supabase,
                    extensionTargetUrl,
                    extensionId,
                    identityRuntime,
            ...
            if (identityRuntime) {
                console.warn(
                    'Falling back to hosted OAuth callback:',
                    oauthTransport.fallbackReason || 'native-browser-oauth-disabled'
                );
            }

            return await this._startHostedGoogleLogin({
                supabase,
                extensionTargetUrl,
                extensionId,
                fallbackReason: oauthTransport.fallbackReason
            });
        } catch (error) {
            this._oauthLoginInProgress = false;
