...
async function loadPagesDropdown() {
    ...
    const popupAccountSyncState = await getPopupAccountSyncState();
    if (!popupAccountSyncState.isLoggedIn || popupAccountSyncState.loginSyncPending) {
    ...
    chrome.runtime.sendMessage({ action: 'getPages' }, async (response) => {
        ...
        const latestPopupAccountSyncState = await getPopupAccountSyncState();
        if (!latestPopupAccountSyncState.isLoggedIn
            || latestPopupAccountSyncState.loginSyncPending
            || response.requiresLogin
            || response.loginSyncPending) {
            resetQuickSavePageDropdown(select, { disabled: true });
            return;
        }
...
async function updateSyncStatus() {
    ...
    if (popupAccountSyncState.isLoggedIn && !popupAccountSyncState.loginSyncPending) {
        // Logged in - hide banner, show save UI
        setSyncBannerState({
            visible: false,
    ...
    } else if (popupAccountSyncState.loginSyncPending) {
        setSyncBannerState({
            visible: true,
            message: 'Syncing your account...',
            showSignInLink: false
        });
        if (initialState) initialState.style.display = 'none';
    } else {
        // Not logged in - show login banner, hide all save UI
        setSyncBannerState({
            visible: true,
            message: 'Please login to save bookmarks',
            showSignInLink: true
