...
function evaluateSubscriptionWriteAccess(subscriptionStatus) {
    ...
    if (subscriptionStatus === 'grace') {
        return {
            ...
            message: 'Subscribe to save bookmarks. You are in read-only mode.'
        };
    }
    ...
    // Unknown/missing status: fail closed so background writes don't violate server RLS.
    return {
        allowed: false,
        requiresStatusRefresh: true,
        message: 'Open LumiList once to refresh account status, then try again.'
    };
}
