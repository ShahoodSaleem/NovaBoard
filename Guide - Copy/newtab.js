...
document.addEventListener('DOMContentLoaded', async function () {
    try {
        ...
        // Early auth check - determine if we should load boards
        // Must check BEFORE rendering any UI to prevent boards showing when logged out
        // Wrapped in try-catch to prevent blank page if auth check fails
        let isLoggedIn = hasTrustedStoredUser;
        if (!isLoggedIn && typeof SyncManager !== 'undefined') {
            try {
                isLoggedIn = await SyncManager.isLoggedIn();
            } catch (error) {
                console.error('Auth check failed during initialization:', error);
                isLoggedIn = false; // Fail gracefully - show welcome screen
            }
        }

        if (isLoggedIn && typeof SyncManager !== 'undefined') {
            const storedUser = hasTrustedStoredUser
                ? initialWallpaperScopeUser
                : await SyncManager.getStoredUser();
            setWorkspaceOwnerUserId(storedUser?.id || null);
        } else {
            setWorkspaceOwnerUserId(null);
        }

        // Show welcome screen immediately if not logged in (don't wait for setupModalEventListeners)
        // Also add body class to hide all interactive UI when not authenticated
        if (isLoggedIn) {
            document.body.classList.remove('not-authenticated');
            beginWorkspaceStartupShell();
            if (welcomeScreen) welcomeScreen.classList.add('hidden');
            ...
            try {
                ...
                console.warn('Failed to preload wallpaper catalog for logged-out welcome screen:', error);
            }
            await resetThemeForLoggedOutState();
            if (welcomeScreen) welcomeScreen.classList.remove('hidden');

            // Reset welcome screen to default button state (prevents stale syncing state after logout reload)
        ...
        try {
            await initializeDatabase({
                shouldLoadBoards: isLoggedIn,
                isLoggedIn,
                startupStorage,
                deferBootstrap: hasPendingAuthRedirect
            });
        } finally {
            if (isLoggedIn) {
                finishWorkspaceStartupShell();
...
function setupAuthEventListeners() {
    // Welcome screen Google button
    const welcomeGoogleBtn = document.getElementById('welcomeGoogleBtn');
    if (welcomeGoogleBtn) {
        welcomeGoogleBtn.addEventListener('click', handleWelcomeGoogleLogin);
    }
 ...
 */
function canModify() {
    const UNKNOWN_STATUS_TOAST_COOLDOWN_MS = 5000;
    const DEGRADED_STATUS_TOAST_COOLDOWN_MS = 15000;
    if (!window._lastUnknownStatusToastAt) window._lastUnknownStatusToastAt = 0;
    if (!window._lastDegradedStatusToastAt) window._lastDegradedStatusToastAt = 0;
    if (!canMutateAccountScopedPreferences()) {
        return false;
    }
    const isAuthenticatedContext = typeof document === 'undefined' ||
        (initialAuthCompleted && !document.body?.classList?.contains('not-authenticated'));
    const status = (
        window.subscriptionStatus === 'trial' ||
        window.subscriptionStatus === 'active' ||
        window.subscriptionStatus === 'grace' ||
        window.subscriptionStatus === 'expired'
    )
        ? window.subscriptionStatus
        : (
            isAuthenticatedContext && (
                window.cachedSubscriptionStatus === 'trial' ||
                window.cachedSubscriptionStatus === 'active' ||
                window.cachedSubscriptionStatus === 'grace' ||
                window.cachedSubscriptionStatus === 'expired'
            )
        )
            ? window.cachedSubscriptionStatus
            : window.subscriptionStatus;

    // Allow modifications for trial and active users
    if (status === 'trial' || status === 'active') {
        return true;
    }

    // Block modifications for grace and expired users
    if (status === 'grace') {
        globalThis.logSubscriptionDebugEvent?.('newtab.subscription.can_modify.block_grace', {
            status,
            subscription: window.subscriptionData || null
        });
        showGlassToast('Subscribe to edit bookmarks. Read-only mode.', 'warning');
        return false;
    }

    if (status === 'expired') {
        if (isAuthenticatedContext && !window.subscriptionData) {
            globalThis.logSubscriptionDebugEvent?.('newtab.subscription.can_modify.defer_expired_without_subscription', {
                status,
                cachedStatus: window.cachedSubscriptionStatus
            });
            showGlassToast('Refreshing subscription status. Please wait a moment.', 'warning');
            return false;
        }
        ...
        if (
            ...
            const derivedStatus = SyncManager.getEffectiveStatus(window.subscriptionData);
            if (derivedStatus === 'trial' || derivedStatus === 'active') {
                globalThis.logSubscriptionDebugEvent?.('newtab.subscription.can_modify.corrected', {
                    originalStatus: status,
                    correctedStatus: derivedStatus,
                    subscription: window.subscriptionData || null
                });
                window.subscriptionStatus = derivedStatus;
                if (typeof SyncManager.getDaysRemaining === 'function') {
                    window.subscriptionDaysLeft = SyncManager.getDaysRemaining(window.subscriptionData, derivedStatus);
                }
                return true;
            }
            if (derivedStatus === 'grace') {
                globalThis.logSubscriptionDebugEvent?.('newtab.subscription.can_modify.corrected_to_grace', {
                    originalStatus: status,
                    correctedStatus: derivedStatus,
                    subscription: window.subscriptionData || null
                });
                window.subscriptionStatus = 'grace';
                if (typeof SyncManager.getDaysRemaining === 'function') {
                    window.subscriptionDaysLeft = SyncManager.getDaysRemaining(window.subscriptionData, 'grace');
                }
                showGlassToast('Subscribe to edit bookmarks. Read-only mode.', 'warning');
                return false;
            }
        ...
        globalThis.logSubscriptionDebugEvent?.('newtab.subscription.can_modify.block_expired', {
            status,
            subscription: window.subscriptionData || null
        });
        showGlassToast(getExpiredSubscriptionMessage(window.subscriptionData), 'error');
        showExpiredOverlay();
        return false;
    }

    // FIX [Issue #24]: Block when status is undefined/null (loading state)
    // This prevents expired users from modifying during the brief window before
    // subscription status is fetched. Silent block to avoid toast spam during init.
    if (status === undefined || status === null) {
        // Network outage safety: when sync/auth is transiently degraded, allow local edits.
        // Cloud push will retry when connectivity recovers, and server RLS still enforces policy.
        const fallbackWritableStatus = window.cachedSubscriptionStatus;
        const hasWritableFallback = fallbackWritableStatus === 'trial' || fallbackWritableStatus === 'active';
        if (isAuthenticatedContext && hasWritableFallback && isTransientSyncDegraded()) {
            const now = Date.now();
            if (now - window._lastDegradedStatusToastAt > DEGRADED_STATUS_TOAST_COOLDOWN_MS) {
                window._lastDegradedStatusToastAt = now;
                showGlassToast('Network issue detected. Changes will be saved locally and synced when connection recovers.', 'warning');
            }
            return true;
        }

        const now = Date.now();
        if (now - window._lastUnknownStatusToastAt > UNKNOWN_STATUS_TOAST_COOLDOWN_MS) {
            window._lastUnknownStatusToastAt = now;
            showGlassToast('Refreshing account status. Please wait a moment and try again.', 'warning');
        }
        return false;
    }

    // Unknown status - default to block for safety
    console.warn('canModify: Unknown status:', status);
    return false;
}
...
async function updateSettingsSubscriptionDisplay() {
    ...
    if (status === 'trial') {
        ...
        infoEl.classList.add('is-warning');
        infoEl.textContent = 'Refreshing account status. Please try again in a moment.';
        infoEl.style.display = 'block';
        subscribeBtn.textContent = 'Refresh';
        subscribeBtn.style.display = 'block';
    }
}
...
`);
    ...
    const url = `https://github.com/sat008sat/lumilist-feedback/issues/new?title=${title}&body=${body}&labels[]=bug&labels[]=user-reported`;
...
function isUserCancelledGoogleLoginError(errorMessage) {
    ...
    return normalizedMessage.includes('login was cancelled before completion') ||
        normalizedMessage.includes('did not approve access') ||
        normalizedMessage.includes('login was canceled before completion');
}

// Handle Google login from welcome screen
// NOTE: Do NOT show syncing state here - only show it when user RETURNS from Google
async function handleWelcomeGoogleLogin() {
    setWelcomeGoogleButtonState({
        visible: true,
        disabled: true,
        loading: true,
        label: WELCOME_GOOGLE_BUTTON_LOADING_LABEL
    });

    try {
        if (pendingLoggedOutThemeResetPromise) {
            await pendingLoggedOutThemeResetPromise;
        }

        const result = await SyncManager.loginWithGoogle();

        if (!result.success && !result.redirecting) {
            // Error - re-enable button and show toast (H4)
            setWelcomeGoogleButtonState({ visible: true, disabled: false, loading: false });
            if (isUserCancelledGoogleLoginError(result.error)) {
                showGlassToast('Google sign-in was cancelled.', 'info');
                return;
            }
            console.error('Google login failed:', result.error);
            showAuthError(result.error || 'Login failed. Please try again.');
        }
        // If redirecting, page will navigate away to Google
    } catch (error) {
