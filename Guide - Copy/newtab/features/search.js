    ...
    features.search = function createSearchFeature(options) {
        ...
        function performSearch(query, config = searchDefaultOptions) {
            ...
            for (const boardEntry of searchState.dataset) {
                const boardNamePhraseMatch = includeBoardName && boardEntry.boardNameNorm.includes(normalizedQuery);
                ...
                for (const bookmark of boardEntry.bookmarks) {
                    const titlePhraseMatch = includeTitle && bookmark.titleNorm.includes(normalizedQuery);
                    const urlPhraseMatch = includeUrl && bookmark.urlNorm.includes(normalizedQuery);
                    const descriptionPhraseMatch = includeNote && bookmark.descriptionNorm.includes(normalizedQuery);
                    ...
                    if (!titleMatched && !urlMatched && !descriptionMatched) continue;
                    ...
                    if (titlePhraseMatch || urlPhraseMatch || descriptionPhraseMatch) {
        ...
        async function activateSearchKeyboardSelection(config = {}) {
            ...
            if (!searchState.keyboardItems || searchState.keyboardItems.length === 0) return;
